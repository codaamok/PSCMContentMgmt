        ...
        switch ($PSCmdlet.ParameterSetName) {
            "Folder" {
                ...
                foreach ($File in $Files) {
                    if ($File.Name -match "^(?<ObjectType>0|3|5|257|258|259|512)_(?<ObjectID>[A-Za-z0-9]+)\.pkgx$") {
                        ...
                        $Command = 'Start-CMContentDistribution -{0} "{1}" -DistributionPointName "{2}" -ErrorAction "Stop"' -f [SMS_DPContentInfo_CMParameters][SMS_DPContentInfo]$InputObject.ObjectType, $InputObject.ObjectID, $Target
            ...
            default {
                ...
                foreach ($Object in $InputObject) {
                    ...
                    $Command = 'Start-CMContentDistribution -{0} "{1}" -DistributionPointName "{2}" -ErrorAction "Stop"' -f [SMS_DPContentInfo_CMParameters][SMS_DPContentInfo]$Object.ObjectType, $Object.ObjectID, $Target
                    $ScriptBlock = [ScriptBlock]::Create($Command)
                    try {
                        if ($PSCmdlet.ShouldProcess(
